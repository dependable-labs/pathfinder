...
pub trait AllocatorProtection<'info> {
  fn is_allocator(&self, user: &Signer, config: &Account<'info, ManagerVaultConfig>, allocator: Option<&Account<'info, AllocatorState>>) -> Result<()> {
    ...
    let has_allocator_permission = allocator
        .map(|alloc| alloc.is_allocator)
        .unwrap_or(false);
