...
impl Decimal {
    ...
    pub fn try_round_u64(&self) -> Result<u64> {
        let rounded_val = Self::half_wad()
            ...
            .ok_or(MarketError::MathOverflow)?;
        Ok(u64::try_from(rounded_val).map_err(|_| MarketError::MathOverflow)?)
    }
    ...
    pub fn try_ceil_u64(&self) -> Result<u64> {
        let ceil_val = Self::wad()
            ...
            .checked_div(Self::wad())
            .ok_or(MarketError::MathOverflow)?;
        Ok(u64::try_from(ceil_val).map_err(|_| MarketError::MathOverflow)?)
    }
...
mod test {
    ...
    fn test_to_scaled_val() {
        assert_eq!(
            Decimal(U256::from(u128::MAX)).to_scaled_val().unwrap(),
            u128::MAX
        );
